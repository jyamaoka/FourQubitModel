//
/// \file src/FourQubitQubitHousing.cc
/// \brief Definition of FourQubitQubitHousing class
/// 
...
// Primary Constructor
FourQubitQubitHousing::FourQubitQubitHousing(G4RotationMatrix * pRot,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
// Default Constructor
FourQubitQubitHousing::FourQubitQubitHousing()
{  
}
...
void FourQubitQubitHousing::ConstructQubitHousing(G4RotationMatrix * pRot,
							     const G4ThreeVector & tLate,
							     const G4String & pName,
							     G4LogicalVolume * pMotherLogical,
							     G4bool pMany,
							     ...
							     G4bool pSurfChk)
{
  ...
  //Start with solid block of copper, then carve out the stuff where there is empty space in the housing.
  //The radial cutouts we are treating as rectangular prisms for now (close enough to reality)
  ...
  //-----------------------------------------------------------------------
  //Now take the radial cutouts and move them to the right position relative to the primary cutout

  //First of eight
  G4RotationMatrix * rot1 = new G4RotationMatrix();
  rot1->rotateZ(45.*deg);
  G4ThreeVector trans1(0.5*dp_housingCentralCutoutDimX, //Move to top right corner
		       0.5*dp_housingCentralCutoutDimY, //Move to top right corner
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  rot2->rotateZ(-45.*deg);
  G4ThreeVector trans2(0.5*dp_housingCentralCutoutDimX, //Move to bottom right corner
		       -0.5*dp_housingCentralCutoutDimY, //Move to bottom right corner
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  rot3->rotateZ(-45.*deg);
  G4ThreeVector trans3(-0.5*dp_housingCentralCutoutDimX, //Move to top left corner
		       0.5*dp_housingCentralCutoutDimY, //Move to top left corner
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  rot4->rotateZ(45.*deg);
  G4ThreeVector trans4(-0.5*dp_housingCentralCutoutDimX, //Move to bottom left corner
		       -0.5*dp_housingCentralCutoutDimY, //Move to bottom left corner
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  rot7->rotateZ(0.*deg);
  G4ThreeVector trans7(0,
		       0.5*dp_housingCentralCutoutDimY, //Move to bottom (Y)
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  rot8->rotateZ(0.*deg);
  G4ThreeVector trans8(0,
		       -0.5*dp_housingCentralCutoutDimY, //Move to top (Y)
		       0.5*(dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ));//Move to top of chip  
  ...
  G4ThreeVector cutoutTranslation(0.0,
				  0.0,
				  (dp_housingDimZ-dp_housingCentralCutoutDimZ)/2.0+dp_eps); //Modifying by one micron so that we're unambiguously removing the top surface of housing
  
  G4SubtractionSolid * solid_qubitHousingNoAddIn = new G4SubtractionSolid("QubitHousingNoAddIn_solid",
									  solid_qubitHousingBlock,
									  solid_totalCutout,
									  0,
									  cutoutTranslation);
  ...
  G4Box * solid_radialCornerAddIn = new G4Box("RadialCornerAddIn_solid",
					      0.5 * dp_housingRadialCutoutDimX,
					      0.5 * dp_housingRadialCutoutDimX,
					      (0.5 * (dp_housingCentralCutoutDimZ - dp_housingRadialCutoutDimZ)));
  ...
  G4UnionSolid * solid_qubitHousingAddIn1 = new G4UnionSolid("QubitHousingAddIn1_solid",
							     solid_qubitHousingNoAddIn,
							     solid_radialCornerAddIn,
							     rot9,
							     addInTranslation1 + cutoutTranslation);

  //Second corner
  G4ThreeVector addInTranslation2(0.5*dp_housingCentralCutoutDimX,
				  -0.5*dp_housingCentralCutoutDimY,
				  -(dp_housingRadialCutoutDimZ/2.0));
  G4UnionSolid * solid_qubitHousingAddIn2 = new G4UnionSolid("QubitHousingAddIn2_solid",
							     solid_qubitHousingAddIn1,
							     solid_radialCornerAddIn,
							     rot9,
							     addInTranslation2 + cutoutTranslation);
  ...
  G4ThreeVector addInTranslation3(-0.5*dp_housingCentralCutoutDimX,
				  -0.5*dp_housingCentralCutoutDimY,
				  -(dp_housingRadialCutoutDimZ/2.0));
  G4UnionSolid * solid_qubitHousingAddIn3 = new G4UnionSolid("QubitHousingAddIn3_solid",
							     solid_qubitHousingAddIn2,
							     solid_radialCornerAddIn,
							     rot9,
							     addInTranslation3 + cutoutTranslation);
  ...
  G4ThreeVector addInTranslation4(-0.5*dp_housingCentralCutoutDimX,
				  0.5*dp_housingCentralCutoutDimY,
				  -(dp_housingRadialCutoutDimZ/2.0));
  G4UnionSolid * solid_qubitHousing = new G4UnionSolid("QubitHousing_solid",
						       solid_qubitHousingAddIn3,
						       solid_radialCornerAddIn,
						       rot9,
						       addInTranslation4 + cutoutTranslation);
  ...
  G4LogicalVolume * log_QubitHousing = new G4LogicalVolume(solid_qubitHousing,
							   copper_mat,
							   pName);

  //Now, create a physical volume and G4PVPlacement for storing as the final output
  G4VPhysicalVolume* phys_QubitHousing = new G4PVPlacement(pRot,
							   tLate,
							   log_QubitHousing,
							   pName, 
							   pMotherLogical,
